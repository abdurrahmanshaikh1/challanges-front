...
// Brands
const brands = ["Zudio", "Zara", "Louis Vuitton"];
...
const userRatings = [
  ...
  const filteredUsers = useMemo(() => {
    const range = userRatings.find((r) => r.label === selectedRatingLabel);
    if (!range || selectedRatingLabel === 'All Ratings') return users;

    return users.filter(
      (u) => u.rating >= range.min && u.rating <= range.max
    );
  }, [selectedRatingLabel, users, userRatings]);
  ...
  const handleRatingChange = (e) => {
    setSelectedRatingLabel(e.target.value);
    setCurrentPage(1);
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
      ...
      {/* Filters bar */}
      <section className="mx-8 mb-6 bg-slate-900/80 border border-slate-800 rounded-xl px-5 py-4">
        <h3 className="text-sm font-medium mb-3 text-slate-200">Filters</h3>
        <div className="w-full max-w-xs">
          <label className="text-xs text-slate-400 block mb-1.5">User Rating</label>
          <select
            value={selectedRatingLabel}
            onChange={handleRatingChange}
            className="w-full bg-slate-950/70 border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-cyan-500/40"
          >
            {userRatings.map((r) => (
      ...
      <main className="px-8 pb-8">
        <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {currentUsers.map((user) => (
            <article
              key={user.id}
              className="bg-slate-900/80 border border-slate-800 rounded-xl px-5 py-4 flex gap-4 items-center shadow-md hover:shadow-xl hover:-translate-y-1 transition-transform duration-200"
            >
              {/* Avatar */}
              ...
              <div className="flex-1 space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1 text-xs text-amber-400">
                    <span>★</span>
                    <span className="text-slate-100">{user.rating}</span>
                  </div>
                </div>
                ...
                <p className="text-xs text-slate-300 flex items-center gap-2">
                  <span className="text-slate-500">📅</span>
                  Joined {new Date(user.joinedDate).toLocaleDateString('en-GB')}
                </p>
              </div>
            </article>
          ))}
        ...
        <div className="flex items-center justify-between mt-6 text-sm text-slate-300">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3 py-1.5 rounded-lg border border-slate-700 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-800/80"
            >
              Prev
            </button>
            <button
